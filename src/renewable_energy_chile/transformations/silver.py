...
def silver_hub_planta_combined():
    ...
    solar_df = get_source("dev.bronze_renewable_energy.bronze_real_solar")
    eolic_df = get_source("dev.bronze_renewable_energy.bronze_real_eolic")
...
def silver_hub_reductions_combined():
    ...
    # Union the two streaming sources
    eolic_df = get_source("dev.bronze_renewable_energy.bronze_reductions_preliminary_eolic")
    solar_df = get_source("dev.bronze_renewable_energy.bronze_reductions_preliminary_solar")
...
def silver_link_measure():
    ...
    df_c = spark.read.table("silver_renewable_energy.silver_hub_coordinated") \
              .select(col("plant_name").alias("coordinated_key")) \
              .distinct()

    df_a = spark.read.table("silver_renewable_energy.silver_hub_planta") \
                .select(col("plant_name").alias("plant_key")) \
                .distinct()

    df_b = spark.read.table("silver_renewable_energy.silver_hub_reductions") \
                .select(col("plant_name").alias("reductions_key")) \
                .distinct()

    # Perform Full Outer Joins
    
    joined_df = df_a.join(df_b, df_a.plant_key == df_b.reductions_key, "full") \
                    .join(df_c, (df_a.plant_key == df_c.coordinated_key) | 
                                (df_b.reductions_key == df_c.coordinated_key), "full")

    # 5. Add record_date
    return joined_df.withColumn("record_date", current_date()) \
