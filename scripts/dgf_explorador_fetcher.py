...
def main() -> int:
    ...
    ap.add_argument("--landing", default=os.environ.get("DGF_LANDING", "./_dgf_landing"),
                    help="output dir / UC Volume path (e.g. /Volumes/<cat>/<schema>/lookup/dgf/)")
...
if __name__ == "__main__":
    # Only signal a non-zero exit on failure — a bare SystemExit(0) is surfaced as an
    # error by the Databricks python-task runner, so don't raise it on success.
    _rc = main()
    if _rc:
